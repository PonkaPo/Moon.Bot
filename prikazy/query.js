...
module.exports.run = async (client, message, args, DBConnection) => {
    if(message.author.id !== "409731934030135306") return message.channel.send("**Query**: You can't access the query command.");
    if(args.length == 0) return message.channel.send("**Query**: You didn't provide any syntax query to check.");
    if(args[0] == "SELECT") {
    ...
    if(getting_query.length === 0) getting_query
    let QueryEmbedSucc = new Discord.MessageEmbed()
        .setTitle("Query")
        .setColor("#F9A3BB")
        .setDescription(answer);
    message.channel.send(QueryEmbedSucc);
};
