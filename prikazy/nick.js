...
module.exports.run = async (client,message,args) => {
  ...
  if (message.mentions.members.first()) {
    mentioneduser = message.mentions.members.first();args.shift();
  } else {
  ...
  if (!args[0]) {
    if ((mentioneduser.roles.highest.position || message.member.roles.highest.position) > message.guild.members.resolve(message.client.user).roles.highest.position) return message.channel.send('**NICK**: Mentioned member '+mentionedusername+' has higher role than me.');
  ...
  try {
    ...
    return;
  } catch(err) { return message.channel.send('**NICK** - Error occured:\n' + err); }
}
module.exports.help = {
  name: 'nick',
