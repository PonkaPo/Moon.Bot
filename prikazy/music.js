...
module.exports.run = async (client,message, args, DBConnection) => {
    ...
    if (!args[0]) return message.reply("**Music**: You didn't write any artist.");
    if (!args[1]) return message.reply("**Music**: You didn't write any song.");

    const artistResult = await sfunctions.check_if_artist_exists(DBConnection, message, args);

    if(artistResult == 0) return message.channel.send("**Music**: Provided artist isn't exists in the database.");

    var musicResult = await sfunctions.check_if_song_exists(DBConnection, message, args);
    if(musicResult.length === 0) return message.channel.send("**Music**: Provided song name isn't exists in the database.");
