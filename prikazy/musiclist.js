...
module.exports.run = async (client,message, args, DBConnection) => {
  const MLEmbed = new Discord.MessageEmbed().setColor('#F9A3BB').setTimestamp().setThumbnail(message.client.user.avatarURL()).setFooter("Pinkamena.Bot | Music", message.client.user.avatarURL({ dynamic: true })).setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }));
  if(args.length == 0) {
    ...
    if(args.length > 1) return message.channel.send("**MusicList**: Artist can't contain spaces.");
    ...
    if(check_for_artist == 0) {
      ...
      if(get_music_result == 0) {
        ...
        return message.channel.send(MLEmbed);
      }
